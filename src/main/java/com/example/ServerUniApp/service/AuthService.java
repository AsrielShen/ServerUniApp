...
public class AuthService {
    ...
    private AuthResponseVO login(Auth auth) {
        if("student".equals(auth.getRole())) {
            Student user = studentMapper.selectByAuthId(auth.getId());
            ...
            Teacher user = teacherMapper.selectByAuthId(auth.getId());
            String token = jwtUtil.getToken(user.getId(),user.getRole());
    ...
    public AuthResponseVO loginByPassword(LoginRequestVO request) {
        //登录认证
        Auth auth = authMapper.selectByUserNumber(request.getUserNumber());
        if (auth == null)
    ...
    public AuthResponseVO loginByOpenid(String openid) {
        Auth auth = authMapper.selectByOpenid(openid);
        if (auth == null)
    ...
    public AuthResponseVO registerStudent(RegisterStudentVO request) {
        ...
        if(authMapper.selectByUserNumber(request.getStudentNumber()) != null) {
